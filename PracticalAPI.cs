...
public class PracticalAPI : PracticalSingleton<PracticalAPI>
{
	...
	* @apiParam (Parameters) {string} interactObject References the GameObject being viewed while stat is being recorded. (AutoGenerated)
	* @apiParamExample {json} Structure:
	...
	*/
	...
	public void RecordCustomStat(string uniqueIdentifier, float value, Measurement measurement = Measurement.Count, Formula formula = Formula.Total, string interactObject = "")
	{
		if (logSummary)
		{
		...
		{
			int ret = PracticalDLL.RecordCustomStat(uniqueIdentifier, value.ToString(), ((int)measurement).ToString(), ((int)formula).ToString(), interactObject);
	...
	* @apiParam (Parameters) {string} interactObject Reference the GameObject being viewed while stat is being recorded. (AutoGenerated)
	* @apiParamExample {json} Structure:
	...
	*/
	...
	public void RecordGesture(string uniqueIdentifier, float HoldLength = 0.0f, GestureType gestureType = GestureType.Tap, string interactObject = "")
	{
		if (logSummary)
		{
		...
		else
		{
			...
			int ret = PracticalDLL.RecordGesture(uniqueIdentifier, "1.0", ((int)gestureType).ToString(), interactObject, HoldLength.ToString());
	...
	* 
	* @apiParam (Parameters) {string} viewedObject Records the object name that is being viewed.
	* @apiParamExample {json} Structure:
	...
	*/
	...
	public void RecordGazeOn(string focusObjectName)
	{
		viewedObject = focusObjectName;
		gazeStopWatch.Start();
	}
